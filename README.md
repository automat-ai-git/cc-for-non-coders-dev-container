...
Контейнер поддерживает два режима работы — через облачный API (Z.AI/GLM) и через локальную модель (Ollama). Переключение требует отдельно запущенного контейнера LiteLLM в той же сети.
