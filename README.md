...
|-----------|------|----------|
| auth-gateway.py | 8989 (внешний) | 8080 (внутренний) Python-прокси с cookie-авторизацией, единая точка входа |
...
## Переключение модели (облако / локальная)

Контейнер поддерживает два режима работы — через облачный API (Z.AI/GLM) и через локальную модель (Ollama).

```bash
# Внутри контейнера (в терминале code-server):
~/switch-model.sh cloud   # облако:  GLM-модели → Z.AI напрямую
~/switch-model.sh local   # локально: ollama-local → Ollama через LiteLLM
~/switch-model.sh         # показать текущий режим (URL + модели)
```
...
**Предусловие для `local`:** требуется запущенный LiteLLM-контейнер в той же Docker-сети, настроенный на Ollama. Ничего менять в `.env` не нужно — `switch-model.sh` сам пропишет нужный URL в `~/.claude/.env`.
