...
- **Claude (Opus 4.8):** 8–12 минут
- **GLM 5.2:** 15–20 минут (300 строк — ощутимый объём; проверьте оценку перед занятием на выданном тарифе)
...
Agent Teams экспериментальны и без этой переменной выключены. В промпте явно попросите создать agent team.
...
Создай agent team из трёх teammates:
...
Это ключевой момент демо. Агент 2, анализируя тональность, замечает, что в сегменте individual все 36 респондентов — детракторы, а 29 комментариев связаны с доставкой. Он пишет агенту 3 (аналитику) напрямую: «Обрати внимание: NPS individual равен −100, а доставка упоминается в 29 из 36 ответов. Проверь, не смешаны ли в выборке два разных клиентских пути.»
...
Агент 3 берёт из работы агента 1 (темы) и агента 2 (тональность) конкретные комментарии для цитирования в отчёте. Например: «Топ-1 проблема: поддержка (41 упоминание, средняя оценка 2.9). Типичная цитата: 'Поддержка закрыла мой тикет как решённый, хотя проблема осталась'.»
...
**Обработка CSV занимает больше 20 минут.** Для короткого демо можно взять первые 100 строк и предупредить студентов, что полный файл обработается дольше.
